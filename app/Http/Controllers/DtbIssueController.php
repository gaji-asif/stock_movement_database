...
class DtbIssueController extends Controller
{
    ...
    public function index($id,Request $request)
    {
        ...
        if(count($formarr['selectStatus'])>0){
         ...
         // start asif's Code
    if(isset($formarr['arrow_value'])){
        
      $explode_arrow_value = explode('_', $formarr['arrow_value']);

      $database_column = $explode_arrow_value[0];
      $arrow_up_down = $explode_arrow_value[1];

        if($database_column == 'id'){
          $database_column_specific = 'id';
        }

        if($database_column == 'i'){
          $database_column_specific = 'issue_title';
        }

        if($database_column == 'appid'){
          $database_column_specific = 'app_id';
        }

        if($database_column == 'c'){
          $database_column_specific = 'category_id';
        }

        if($database_column == 'version'){
          $database_column_specific = 'version_id';
        }

        if($database_column == 'userid'){
          $database_column_specific = 'user_id';
        }

        if($database_column == 'status'){
          $database_column_specific = 'status';
        }

        if($database_column == 'issuepriorityid'){
          $database_column_specific = 'issue_priority_id';
        }

        if($database_column == 'updatedat'){
          $database_column_specific = 'updated_at';
        }
      ...
      $query .= "ORDER BY i.$database_column_specific $asec_desc";
        ...
        $categories = DtbIssueCategory::where('developer_id',$developer_id)->orderBy('ordering','ASC')->get();
        //$priorities = DtbIssuePriority::where('project_id',$id)->get();
        $priorities = DtbIssuePriority::where('developer_id',$developer_id)->orderBy('ordering','ASC')->get();
       ...
       // $issueStatuss = DtbIssueStatus::getIssueStatus($id);
        $issueStatuss = DtbIssueStatus::where('developer_id',$developer_id)->where('is_true',1)->orderBy('ordering','ASC')->get();
    ...
    public function issue_search_by_sort_project_wise(Request $request)
    {     

        $arrow_value = $_POST['arrow_value'];
...
$explode_arrow_value = explode('_', $arrow_value);
$database_column = $explode_arrow_value[0];
$arrow_up_down = $explode_arrow_value[1];
...
if(isset($arrow_value)){
  $query .= "ORDER BY i.$database_column_specific $asec_desc LIMIT 10";
          //$formArry['selectAssignee'] = $request->selectAssignee;
  $search_key .= ",arrow_value:".$arrow_value;
}
 ...
 // End for Sorting
   ...
   return view('issue.all_issues_projects',compact('id','notInIssue', 'issueslist', 'assignees', 'categories', 'priorities', 'issueStatuss','apps','formarr','common_array'));
...
//     $apps = DtbApp::where('project_id',$id)->get();
    ...
    $statuses = DtbIssueStatus::whereRaw("developer_id ='".$loggedindeveloper."' AND (status_name='New' or status_name='new' or status_name='新規')" )->take(1)->get();
    if (count($statuses)== 0 ){
    ...
    $common_array = array(
        'content_heading' => 'Issue Create'
    );



    $priorities = DtbIssuePriority::where('developer_id',$loggedindeveloper)->orderBy('ordering','ASC')->get();
    $projectchcklist = DtbChecklist::where('project_id',$id)->get();
    $project = DtbProject::where('id',$id)->first();
    return view('issue.create',compact('userlists','developgroups','common_array','apps','categories','versions','statuses','users','projects','priorities','id','issueTypes','projectchcklist'));
...
public function store(Request $request,$id)
{
    ...
    //get checklist item of selected checklist title
    if (isset($request->chcklisttitle)) {

        $chcklistitemsofselected = \DB::table('dtb_checklist_details')
        ->select('dtb_checklist_details.id as detailid','dtb_checklist_details.checklist_id','dtb_checklist_details.details')
        ->join('dtb_checklists','dtb_checklist_details.checklist_id','=','dtb_checklists.id')
        ->where('dtb_checklist_details.checklist_id', $request->chcklisttitle)
        ->orderBy('dtb_checklist_details.id', 'ASC')
        ->get();
    ...
    $lastinsertedissueid = $dtbIssueRec->id;
    //insert checklist to chcklist item table of issue
    if (isset($chcklistitemsofselected)) {
        foreach ($chcklistitemsofselected as $chckdtailitems) {

            $insertissuechcklist = DB::table('dtb_issue_checklist_items')->insert(
                [
                    'issue_id' => $lastinsertedissueid, 
                    'parent_chcklist_id' => $request->chcklisttitle, 
                    'details' =>  $chckdtailitems->details,
                    'is_checked' => 0,
                    'status' => 0,
                    'created_at' => now(),
                    'updated_at' => now()
...
public function show($issue_id,Request $request)
{
...
public function deleteIssueView($issue_id,Request $request){
        //redirect to login page with running visited page url; ### statt
    $visitedpage = $request->fullUrl();
    if (!Session()->has('user_id')) {
        return redirect('login')->with('url', $visitedpage);
...
public function deleteIssue($issue_id,Request $request){
        //redirect to login page with running visited page url; ### statt
    $visitedpage = $request->fullUrl();
    if (!Session()->has('user_id')) {
        return redirect('login')->with('url', $visitedpage);
...
public function copyIssue($appid,Request $request)
{
        //redirect to login page with running visited page url; ### statt
    $visitedpage = $request->fullUrl();
    if (!Session()->has('user_id')) {
    ...
    return view('issue.create',compact('dtbissue','userlists','developgroups','apps','categories','versions','statuses','users','projects','priorities','id', 'appid','issueTypes','copyFlag','common_array','projectchcklist'));

}



public function editIssueNew($issue_id,$div,Request $request){
...
//     $apps = DtbApp::where('project_id',$dtbissue->project_id)->get();
    ...
    // $statuses = DtbIssueStatus::where('project_id',$dtbissue->project_id)->where('is_true',1)->orderBy('ordering','ASC')->get();
     $statuses = DtbIssueStatus::where('developer_id',$developer_id)->where('is_true',1)->orderBy('ordering','ASC')->get();
    $users = \DB::table('dtb_users')
    ->join('dtb_users_projects','dtb_users.id','=','dtb_users_projects.user_id')
    ->where('dtb_users_projects.project_id', $dtbissue->project_id)
        //->where('dtb_users.is_archived', 0)
    ...
    return view('issue.editIssueNew',compact('div','dtbissue','userlists','developgroups','apps','categories','versions','statuses','users','projects','priorities','id','issueTypes','commentcontent','feedbacks','projectchcklist'));
}
...
public function updateIssueData(Request $request, $issue_id){
    ...
    $dtbissue->chcklisttitle = $request->get('chcklisttitle');
...
//issue checklist update #####################################
        ...
        if (isset($request->chcklisttitle)) {
            ...
            $findissueidinchktbl = \DB::table('dtb_issue_checklist_items')
            ->select('issue_id')
            ->where('issue_id',  $issue_id)
            ->where('parent_chcklist_id',  $request->chcklisttitle)
            ->get();

            if (!$findissueidinchktbl->isEmpty()) {

            }else
            {

                //delete if have other list of this issue
                // $srchissue = \DB::table('dtb_issue_checklist_items')
                // ->select('issue_id')
                // ->where('issue_id',  $issue_id)
                // ->get();

                //if (!$srchissue->isEmpty()) {
                $dieleteissuerecord = \DB::table('dtb_issue_checklist_items')
                ->where('issue_id',  $issue_id)
                ->delete();
                //}else{}

                
                //get the chcklist based on chklist id
                $chcklistitemsofselected = \DB::table('dtb_checklist_details')
                ->select('dtb_checklist_details.id as detailid','dtb_checklist_details.checklist_id','dtb_checklist_details.details')
                ->join('dtb_checklists','dtb_checklist_details.checklist_id','=','dtb_checklists.id')
                ->where('dtb_checklist_details.checklist_id', $request->chcklisttitle)
                ...
                //insert newly chcklist for this issue
                if (isset($chcklistitemsofselected)) {
                    foreach ($chcklistitemsofselected as $chckdtailitems) {
                        $insertissuechcklist = DB::table('dtb_issue_checklist_items')->insert(
                            [
...
//echo $request->get('mainissueid');die();
    ...
    ->leftjoin('dtb_users as uuu','i.next_user_id', '=', 'uuu.id')
    ->leftjoin('dtb_issue_statuses as ss','i.next_kick_status', '=', 'ss.id')
    ->where('i.id', $request->get('mainissueid'))
    ->get(['ii.issue_title as nextissuetitle','uuu.name as nextusername','ss.status_name as nextstatus','i.next_user_id as nxtuseridofissue','i.next_kick_status as nxtstatsofissue'])
    ->first();
...
public function getissuechcklist(Request $request){


    $checklistdetailsofissue = DB::table('dtb_issue_checklist_items')
    ->where('issue_id',$request->issueid)
    ->orderBy('ordering','ASC')
    ->get();
    $html = "";
    foreach ($checklistdetailsofissue as $issuechecklistdetail) {
        $html .= '<li class="singlechcklist row1 '.( $issuechecklistdetail->is_checked == '1' ? 'strike' : '') .'" data-listid="'.$issuechecklistdetail->id.'" data-ischecked="'.$issuechecklistdetail->is_checked.'" data-id="'.$issuechecklistdetail->id.'" ><span class="checkbox"></span><p class="dtailchckp_'.$issuechecklistdetail->id.'" class="listli">'.$issuechecklistdetail->details.'</p>
        <input class="inlineinput_'.$issuechecklistdetail->id.'" style="display:none" type="text" value="'.$issuechecklistdetail->details.'">
        <div class="iconholder">
        <i class="fas fa-th-list d-block"></i>
        </div>
        </li>';
    }
    return $html ;
...
public function ischeckeditem(Request $request){

    echo $chckid = $request->chckid;
    $chckstats = $request->chckstatus;

    if (isset($chckid)) {
        if (isset($chckstats)) {
            if ($chckstats == 1) {
                $updateischeck = \DB::table('dtb_issue_checklist_items')->where('id',$chckid)->update(['is_checked' => 0]);
                return 0;
            }else{
                $updateischeck = \DB::table('dtb_issue_checklist_items')->where('id',$chckid)->update(['is_checked' => 1]);
                return 1;
            }
        }
    }
...
public function updateOrder(Request $request){
    ...
    $issuechckitems = DtbIssueChecklistItem::where('issue_id',$request->issueid)->get();
    foreach ($issuechckitems as $issuechckitem) {
        $issuechckitem->timestamps = false; // To disable update_at field updation
        $id = $issuechckitem->id;
        foreach ($request->order as $order) {
            if ($order['id'] == $id) {
                $issuechckitem->update(['ordering' => $order['position']]);
            }
        }
...
public function addissuechckitem(Request $request)
{
  ...
  DB::table('dtb_issue_checklist_items')->insert([
     [
         'details' => $request->dfieldval, 
