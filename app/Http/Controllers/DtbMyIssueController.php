...
//        }
    ...
    if(isset($formArry['selectAssignee'])){
      $query .= "AND i.user_id = '".$formArry['selectAssignee']."'";
        ...
        // start asif's Code
    if(isset($formArry['arrow_value'])){
     
      $explode_arrow_value = explode('_', $formArry['arrow_value']);
      ...
      if($database_column == 'p'){
        $database_column_specific = 'project_id';
      }
      ...
      if($database_column == 'progress'){
        $database_column_specific = 'progress';
      }
      ...
      if($database_column == 'deadline'){
        $database_column_specific = 'deadline';
      }
...
public function issue_search_by_sort(Request $request){


 $selectProject = $_POST['selectProject'];
 $selectAssignee = $_POST['selectAssignee'];
 ...
 $search_key="";
 if($selectProject>0){
  $query .= "AND i.project_id = '$selectProject'";
  $formArry['selectProject'] = $selectProject;
  $search_key .= "selectProject:".$selectProject;
}
...
if($is_complete == 1){
 $query .= "AND i.is_closed  = '1'";
 $formArry['is_complete'] = $is_complete;
 $search_key .= ",is_complete:".$is_complete;
}else{
 $query .= "AND i.is_closed  = '0'";
}
...
if(isset($selectAssignee)){
  $query .= "AND i.user_id = '$selectAssignee'";
  $formArry['selectAssignee'] = $selectAssignee;
  $search_key .= ",selectAssignee:".$selectAssignee;
}
...
return view('all_issues_my_issues',compact('issueslist', 'assignees', 'projects','formArry'));
