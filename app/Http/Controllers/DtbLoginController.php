...
class DtbLoginController extends Controller
{
    ...
    public function checkLogin(Request $request) {
        ...
        $results = DtbUser::select('dtb_users.*','dtb_develop_groups.time_zone_id as developertimezone')->where('email', '=', $email)
        ->where('password', '=', $password)
        ->where('verified', '=', '1')
        ->leftJoin('dtb_develop_groups', 'dtb_users.developer_id', '=', 'dtb_develop_groups.id')
        ->first();
