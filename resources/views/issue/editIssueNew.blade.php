...
/*#autoassignsrchrslt.dataTable thead th:last-child {
  ...
  <div class="col-lg-10 mt-4 issueeditholder" style="margin:0 auto">
    {!! Form::open(['route' => ['update_issue_data', $dtbissue->id], 'method' => 'PUT','files' => true, 'enctype' => 'multipart/form-data', 'class' => 'form-horizontal', 'id' => 'edit_issue_and_comment'])!!}
     <div class="row px-0 mb-4 pull-right">
         <strong><a href="{{url('issue/'. $dtbissue->id.'/'.$div)}}" class="mr-2"><div class="btn-success mt-1 btn-sm  mr-2 pull-right dtb_custom_btn_default">view</div></a></strong>
    ...
    <div class="row px-0 mb-4">
      <div class="col-lg-8">
        ...
        <small>
          ...
          <input name="issue_title" type="text" class="col-10 col-lg-12 controls form-control {{ $errors->has('issue_title') ? ' is-invalid' : '' }}" placeholder="Subject title" value="{{ old('issue_title',$dtbissue->issue_title) }}" required style="font-size: 16px;float: left; margin-left: -10px;">
          <br>
    ...
    <!-- <div class="card pb-4 mb-2 mt-4 bg-transparent no_border"> -->
      ...
      <div class="row px-0">
        <div class="col-md-6">
          <ul class="appinfobox pointergroup">
            <li><a href="javascript:void(0)" class="text-body font-weight-semibold">Issue Type</a></li>
            <li><span class="appvalue">

              <select name="issue_type" id="issuetype" class="custom-select {{ $errors->has('issue_type') ? ' is-invalid' : '' }}" style="">
                <option value="">Select Issue Type</option>
                @foreach($issueTypes as $issueType)
                <option  points="{{$issueType->points }}"  value="{{ $issueType->id }}" {{ old('issue_type') == $issueType->id || $dtbissue->issue_type == $issueType->id ? 'selected':'' }}>{{ $issueType->issue_type }}</option>
                @endforeach
              </select>
        ...
        <ul class="appinfobox">
          ...
          <li><span class="appvalue">
           <div class="pointergroup">
            <select name="category_id" id="catg" class="custom-select {{ $errors->has('category_id') ? ' is-invalid' : '' }}">
              <option value="">Select Category</option>
              @foreach($categories as $category)
              <option points="{{ $category->points }}" value="{{ $category->id }}" {{ old('category_id') == $category->id || $dtbissue->category_id == $category->id ? 'selected':'' }}>{{ $category->category_name }}</option>
              @endforeach
            </select>
            <input type="hidden" name="next_kick_status" value="{{ $dtbissue->next_kick_status ?? '' }}">
          </div>
    ...
    <ul class="appinfobox">
      ...
      <li><span class="appvalue">
        ...
        <div class="">
          ...
          <input type="text"  id="estimatehour1"  name="estimate_hour1" class="form-control  {{ $errors->has('estimate_hour1') ? ' is-invalid' : '' }}" placeholder="Estimate Hour One" value="{{ empty(old('estimate_hour1'))?$dtbissue->estimate_hour1:old('estimate_hour1')  }}">
          @endif
          @endif
        </div>
  ...
  <div class="col-md-6">
    <ul class="appinfobox">
      ...
      <li><span class="appvalue">
        ...
        <div class="pointergroup">
         <select name="difficulty"  id="diffic"  class="custom-select {{ $errors->has('difficulty') ? ' is-invalid' : '' }}">
          <option value="0"  {{ old('difficulty') == 0|| $dtbissue->difficulty == 0 ? 'selected':'' }} >Select Difficulty</option>
          <option points="1"  value="1"  {{ old('difficulty') == 1|| $dtbissue->difficulty == 1 ? 'selected':'' }} >Easy</option>
          <option points="2"  value="2"  {{ old('difficulty') == 2|| $dtbissue->difficulty == 2 ? 'selected':'' }}>Medium</option>
          <option points="4" value="3"  {{ old('difficulty') == 3|| $dtbissue->difficulty == 3 ? 'selected':'' }}>Hard</option>
        </select>
...
<div class="col-md-6">
  <ul class="appinfobox">
    <li> <a href="javascript:void(0)" class="text-body font-weight-semibold">Check List</a></li>
    <li>
      <span class="appvalue">
        <div class="">
          <select name="chcklisttitle" class="custom-select {{ $errors->has('chcklisttitle') ? ' is-invalid' : '' }}">
            <option value="">Select Checklist</option>
            @foreach($projectchcklist as $chcklisttitle)
            <option value="{{ $chcklisttitle->id }}" {{ old('chcklisttitle') == $chcklisttitle->id || $dtbissue->chcklisttitle == $chcklisttitle->id ? 'selected':'' }} >{{ $chcklisttitle->title }}</option>
            @endforeach
          </select>
        </div>
      </span>
...
<div class="row px-0">
 <div class="col-md-6">
  <ul class="appinfobox">
    <li> <a href="javascript:void(0)" class="text-body font-weight-semibold">Complete Date</a></li>
    <li><span class="appvalue">
      <div class="">
        <input type="text" data-date="" data-date-format="YYY MMMM DD" name="complete_date" id="complete_date" class="form-control  {{ $errors->has('complete_date') ? ' is-invalid' : '' }}" placeholder="Complete Date" value="<?php

        if(empty($dtbissue->complete_date)) {
          echo old('complete_date');
        }
        else{
          echo date('Y-m-d', strtotime($dtbissue->complete_date));
        }
...
<script>
        ...
        $('.pointergroup select').change(function() {
  // $("#estimatehour1").val($('#issuetype option:selected').attr('points') * $('#catg option:selected').attr('points') * $('#diffic option:selected').attr('points'));
  pointscalc();
});
    ...
    var issuetype = $('#issuetype option:selected').attr('points') || 0;
    var catg = $('#catg option:selected').attr('points') || 0;
    var diffic = $('#diffic option:selected').attr('points') || 0;
    //var pointsum = parseInt(issuetype, 10) + parseInt(catg, 10) + parseInt(diffic, 10);
    var pointsum = ((issuetype * 1) * (catg * 1) * (diffic * 1));
    $("#estimatehour1").val(pointsum);
  }
